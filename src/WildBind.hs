...
updateGrab :: (FrontInputDevice f i, Eq i) => f -> GrabSet i -> GrabSet i -> IO ()
...
wildBind :: (FrontInputDevice fi i, FrontEventSource fe s i, Ord i) => fi -> fe -> Binding s i -> IO ()
...
updateWBState :: (FrontInputDevice f i, Eq i) => f -> Binding s i -> s -> WBState s i ()
...
updateFrontState :: (FrontInputDevice f i, Eq i) => f -> s -> WBState s i ()
updateFrontState front after_state = do
  (cur_binding, _) <- State.get
  updateWBState front cur_binding after_state

updateBinding :: (FrontInputDevice f i, Eq i) => f -> Binding s i -> WBState s i ()
...
wildBindWithState :: (FrontInputDevice fi i, FrontEventSource fe s i, Ord i) => fi -> fe -> WBState s i ()
